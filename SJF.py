...
###
readyQueue = sorted(readyQueue, key=lambda x: (x.burst, x.priority), reverse=False)
...
while True:
    ...
    for i in waitingQueue:
        i.waitedTime += 1
    sorted(waitingQueue, key=lambda x: x.priority)  # sort kardane priority Queue
    for i in waitingQueue:  # aging
        if i.waitedTime % 4 == 0: i.priority -= 1
    threadLock.release()
    event1.set()
    # time.sleep(3)
    for t in threads:
